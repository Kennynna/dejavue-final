...
export function CartOrderNotice() {
  return (
    <Alert className="mt-4 border-border bg-secondary/30 text-left sm:mt-6">
      ...
      <AlertTitle className="text-foreground">Оформление онлайн скоро появится</AlertTitle>
      <AlertDescription className="mt-2 text-sm leading-relaxed text-muted-foreground">
        Пока заказ через сайт недоступен. Позвоните нам — назовите ароматы из корзины, и мы подскажем наличие,
        доставку и способ оплаты.
      </AlertDescription>
      <a
        href={PHONE_HREF}
